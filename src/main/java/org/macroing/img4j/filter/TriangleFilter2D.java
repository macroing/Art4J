...
public final class TriangleFilter2D extends Filter2D {
	...
	public String toString() {
		return String.format("new TriangleFilter2D(%s, %s)", Strings.toNonScientificNotationJava(getResolutionX()), Strings.toNonScientificNotationJava(getResolutionY()));
	}
	...
	public boolean equals(final Object object) {
		if(object == this) {
			...
			return false;
		} else if(!Doubles.equals(getResolutionX(), TriangleFilter2D.class.cast(object).getResolutionX())) {
			return false;
		} else if(!Doubles.equals(getResolutionY(), TriangleFilter2D.class.cast(object).getResolutionY())) {
			return false;
		} else {
			return true;
		}
	...
	public double evaluate(final double x, final double y) {
		return Doubles.max(0.0D, getResolutionX() - Doubles.abs(x)) * Doubles.max(0.0D, getResolutionY() - Doubles.abs(y));
	}
