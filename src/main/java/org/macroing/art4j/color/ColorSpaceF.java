...
public final class ColorSpaceF {
	...
	/**
	 * A {@code ColorSpaceF} instance that represents the identity color space.
	 */
	public static final ColorSpaceF IDENTITY = new ColorSpaceF(1.0F, 1.0F, 1.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F, 1.0F);
	...
	public ColorSpaceF(final float breakPoint, final float gamma, final float xR, final float yR, final float xG, final float yG, final float xB, final float yB, final float xW, final float yW) {
		...
		this.slope = breakPoint > 0.0F ? 1.0F / (gamma / Floats.pow(breakPoint, 1.0F / gamma - 1.0F) - gamma * breakPoint + breakPoint) : 1.0F;
		this.slopeMatch = breakPoint > 0.0F ? gamma * this.slope / Floats.pow(breakPoint, 1.0F / gamma - 1.0F) : 1.0F;
		this.slopeMatchReciprocal = 1.0F / this.slopeMatch;
		this.slopeReciprocal = 1.0F / this.slope;
		this.segmentOffset = breakPoint > 0.0F ? this.slopeMatch * Floats.pow(breakPoint, 1.0F / gamma) - this.slope * breakPoint : 0.0F;
	...
	private float doRedoGammaCorrection(final float value) {
		return value <= this.breakPoint ? value * this.slope : this.slopeMatch * Floats.pow(value, this.gammaReciprocal) - this.segmentOffset;
	}
	
	private float doUndoGammaCorrection(final float value) {
		return value <= this.breakPoint * this.slope ? value * this.slopeReciprocal : Floats.pow((value + this.segmentOffset) * this.slopeMatchReciprocal, this.gamma);
	}
