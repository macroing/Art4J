...
public final class LanczosSincFilter2F extends Filter2F {
	...
	public String toString() {
		return String.format("new LanczosSincFilter2F(%s, %s, %s)", Strings.toNonScientificNotationJava(getResolutionX()), Strings.toNonScientificNotationJava(getResolutionY()), Strings.toNonScientificNotationJava(this.tau));
	}
	...
	public boolean equals(final Object object) {
		if(object == this) {
			...
			return false;
		} else if(!Floats.equals(getResolutionX(), LanczosSincFilter2F.class.cast(object).getResolutionX())) {
			return false;
		} else if(!Floats.equals(getResolutionY(), LanczosSincFilter2F.class.cast(object).getResolutionY())) {
			return false;
		} else if(!Floats.equals(this.tau, LanczosSincFilter2F.class.cast(object).tau)) {
			return false;
		} else {
			return true;
		}
	...
	@Override
	public int hashCode() {
		return Objects.hash(Float.valueOf(getResolutionX()), Float.valueOf(getResolutionY()), Float.valueOf(this.tau));
	}
	
	////////////////////////////////////////////////////////////////////////////////////////////////////
	
	private static float doLanczosSinc(final float x, final float tau) {
		final float xAbs = Floats.abs(x);
		...
		final float xAbsPi = xAbs * Floats.PI;
		final float y = xAbsPi * tau;
		final float sinc = Floats.sin(y) / y;
		final float lanczos = Floats.sin(xAbsPi) / xAbsPi;
